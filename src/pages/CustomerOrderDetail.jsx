...
const CustomerOrderDetail = () => {
  ...
  const handleBack = () => {
    // Fixed to go back to customer orders page instead of admin orders
    navigate('/customer');
  };
  ...
  const handleReorder = () => {
    ...
    const cartItems = orderDetails.items.map(item => ({
      ...item,
      id: Math.random().toString(36).substr(2, 9) // Generate unique ID for cart item
    }));
    
    // You would likely have a cart context or local storage to handle this
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      {/* Sidebar - Fixed to use CustomerSidebar instead of AdminSidebar */}
