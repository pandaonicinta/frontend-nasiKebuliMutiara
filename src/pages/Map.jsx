...
const MapComponent = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/api/dashboard/order`, {
        headers: {
      ...
      .then((response) => {
        let responseData = response.data
          .filter(
            (item) =>
              item.alamat &&
              item.alamat.latitude &&
              item.alamat.longitude &&
              item.status !== "pending"
          )
          .filter((item) => {
          ...
          .map((item) => ({
            ...
            }</b> <br /> Customer a.n. ${item.nama_pembeli}<br />${
              item.alamat.label_alamat
            } - ${item.alamat.nama_penerima} <br />${item.alamat.detail}`,
        ...
        if (statusFilter) {
          responseData = responseData.filter(
            (marker) => marker.status === statusFilter
          );
        }
      ...
      >
        ...
        {markers
          ...
          .map((marker, index) => (
            ...
            >
              <Popup>
                <div dangerouslySetInnerHTML={{ __html: marker.label }} />
              </Popup>
            </Marker>
          ))}
        <MarkerClusterGroup>
