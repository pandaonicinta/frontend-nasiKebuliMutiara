...
const API_BASE_URL = "http://eggsperts.my.id";
...
const MenuDetail = () => {
  ...
  const calculateAverageRating = () => {
    ...
    const totalRating = ulasanList.reduce(
      (sum, ulasan) => sum + parseFloat(ulasan.rating_value),
      0
    );
      ...
      const fetchUlasan = async () => {
        try {
          ...
          const filteredUlasan = response.data.filter(
            (ulasan) =>
              ulasan.id_produk === parseInt(id) ||
              ulasan.produk_id === parseInt(id)
          );
    ...
    const fetchMenuData = async () => {
      try {
        ...
        if (currentProduct.ukuran) {
          const sizes = currentProduct.ukuran
            .split(",")
            .map((size) => size.trim());
          if (sizes.length > 0) {
        ...
        const related = allProducts
          .filter(
            (product) =>
              product.kategori === currentProduct.kategori &&
              product.produk_id !== currentProduct.produk_id
          )
          .slice(0, 3);
      ...
      if (menu) {
        try {
          ...
          const filteredProducts = response.data
            .filter(
              (product) =>
                product.kategori === menu.kategori &&
                product.produk_id !== menu.produk_id
            )
            .slice(0, 3);
  ...
  const addToLocalStorageCart = () => {
    ...
    const existingItemIndex = existingCart.findIndex(
      (item) =>
        item.produk_id === menu.produk_id && item.ukuran === selectedSize
    );
      ...
      if (isAuthenticated) {
        try {
          ...
          if (apiError.response && apiError.response.status === 401) {
            ...
            alert(
              "Terjadi kesalahan saat menambahkan ke keranjang. Menggunakan penyimpanan lokal."
            );
            addToLocalStorageCart();
            addToCart(cartItem);
          }
  ...
  const sizes = menu.ukuran
    ? menu.ukuran.split(",").map((size) => size.trim())
    : [];
      ...
      <div className="px-6 md:px-16 lg:px-32">
        <div className="container mx-auto py-4 bg-white">
          <div className="flex items-center text-sm">
            <a href="/menu" className="text-yellow-500 hover:text-gray-300">
              Menu
            </a>
                ...
                {!isAuthenticated && (
                  <div className="mb-4">
                    <p className="text-amber-600 text-sm">
                      <span className="font-bold">Note:</span> You are not
                      logged in. Items will be saved to local storage.
                  ...
                  >
                    ...
                    {isAuthenticated ? (
                      <span className="block mt-1 text-sm">
                        Menu disimpan di akunmu.
                      </span>
                    ) : (
                      <span className="block mt-1 text-sm">
                        Menu disimpan di local storage.{" "}
              ...
              <div className="flex justify-center">
                {["Deskripsi", "Detail", "Ulasan"].map((tab) => (
                  <button
                    key={tab}
                    onClick={() =>
                      setActiveTab(tab === "Detail" ? "Additional Info" : tab)
                    }
                        ...
                        {Object.entries(additionalInfo).map(
                          ([key, value], index) => (
                            <tr
                              ...
                              className={
                                index <
                                Object.entries(additionalInfo).length - 1
                                  ? "border-b border-gray-300"
                                  : ""
                              }
                            >
                              <td className="py-2 font-bold text-gray-500 capitalize w-1/3">
                                {key}
                              </td>
                      ...
                      <div className="text-center py-12">
                        <div className="mb-6">
                          ...
                          <p className="text-gray-500 max-w-md mx-auto">
                            ...
                            <strong>{menu.nama_produk}</strong> ini dan bantu
                            pelanggan lain membuat keputusan!
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div>
                              ...
                              <div className="flex items-start mb-3">
                                <div className="flex-1 pr-20">
                                  <div className="flex items-center space-x-3 mb-2">
                                    ...
                                    <div
                                      className={`w-8 h-8 bg-[#FDC302] rounded-full flex items-center justify-center ${
                                        ulasan.gambar ? "hidden" : ""
                                      }`}
                                    >
                                      <span className="text-white text-xs font-bold">
                                        {ulasan.name
                                          ? ulasan.name.charAt(0).toUpperCase()
                                          : "U"}
                                      </span>
                                    </div>
                                    <div>
                        ...
                        {ulasanList.length > reviewsPerPage && (
                          <div className="mt-6 text-center text-gray-500 text-sm">
                            Menampilkan {reviewsPerPage} dari{" "}
                            {ulasanList.length} ulasan untuk produk ini
                          </div>
                        )}
                      </div>
                    )}
        ...
        <div className="container mx-auto py-12">
          ...
          <p className="text-center text-gray-500 mb-8 text-sm">
            Pilih dari beberapa menu terkait
          </p>
