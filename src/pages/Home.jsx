...
// Optional: Add authorization header for protected routes
...
const Home = () => {
  ...
  // Fetch products to be used as categories
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // If not, you'll need to adapt this to your API response structure
        const productsData = response.data.data || [];
        
        // Limit to first 4 products for categories display
        const firstFourProducts = productsData.slice(0, 4);
        
        // Map to required format with slugs
        const formattedCategories = firstFourProducts.map(product => ({
          id: product.id,
          name: product.nama || "Nasi Kebuli",
          image: product.gambar || foto,
          slug: product.nama.toLowerCase().replace(/\s+/g, '-') || "nasi-kebuli"
        }));
  ...
  useEffect(() => {
    const fetchPopularProducts = async () => {
      try {
        const response = await axios.get('/produk');
        // For demonstration, we'll use the first 3 products
        // In a real app, you might want to sort by rating or popularity
        const productsData = response.data.data || [];
  ...
  useEffect(() => {
    // In a real app, you would fetch these from an API endpoint
    setTestimonials([
      {
        id: 1,
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      // Assuming API expects productId and quantity
  ...
  const handleAccountNavigation = () => {
    ...
    if (token) {
      ...
      if (userRole === 'admin') {
        ...
        console.log("Unknown user role:", userRole); // Debugging
        navigate('/login');  
      }
    } else {
      navigate('/login');
...
// Make sure you're setting the userRole when logging in
...
const handleLogin = async (credentials) => {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Categories Section - Updated with React Router Links and API Data */}
      <section className="py-16 bg-white">
        <div className="container mx-auto px-8">
          ...
          {/* Updated Card Grid with data from API */}
          {!isLoading && !error && (
