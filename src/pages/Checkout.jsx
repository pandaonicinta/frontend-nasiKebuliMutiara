...
const getImageUrl = (imagePath) => {
  ...
  return `http://eggsperts.my.id/storage/${imagePath}`;
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
            ...
            {addresses && addresses.length > 0 ? (
              addresses.map((address) => (
                <div
                  ...
                  tabIndex={0}
                  onKeyPress={(e) => {
                    if (e.key === "Enter") onSelectAddress(address.alamat_id);
                  }}
                >
                  <div className="flex justify-between items-center">
                    <div className="text-maroon-700 text-sm font-medium">
        ...
        <div className="p-3 border-t flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-3 py-1.5 border border-gray-300 text-gray-700 text-sm rounded-md hover:bg-gray-50 transition"
          >
            Batal
          </button>
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-yellow-400 text-black text-sm rounded-md hover:bg-yellow-500 shadow-sm transition"
          >
            Konfirmasi
          </button>
        </div>
...
const Checkout = () => {
  ...
  const { cartItems, calculateTotal, clearCart, cartCount } =
  ...
  const handlePaymentSuccess = async (transactionId) => {
    ...
    try {
      ...
      console.log(
        "[handlePaymentSuccess] Token found, making keepalive request..."
      );
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
        ...
        if (cartResponse.data && Array.isArray(cartResponse.data)) {
          checkoutItems.forEach((item) => {
            ...
            const bestMatch = cartResponse.data.find(
              (cartItem) =>
                String(cartItem.id_produk) === prodId &&
      ...
      const payload = {
        ...
        jenis_pembayaran:
          formData.paymentMethod === "cash" ? "tunai" : "transfer",
        id_item: cartItemIds,
      };
  ...
  const selectedAddressDetails =
    addresses.find((addr) => addr.alamat_id === selectedAddress) || null;
      ...
      <header className="bg-white py-4 px-4 md:px-8 lg:px-12 shadow-sm">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-6">
            <a
              href="/"
              className="text-gray-800 hover:text-[#FDC302] font-medium"
            >
              Home
            </a>
            <a
            ...
            <a
              href="/cart"
              className="text-gray-800 hover:text-[#FDC302] relative"
            >
              ...
              {selectedAddressDetails ? (
                <div className="border border-yellow-400 bg-yellow-50 rounded-lg p-4 flex justify-between items-start">
                  <div>
                    <div className="flex items-center space-x-3">
                      <span className="text-red-800 font-semibold text-s">
                        {selectedAddressDetails.label_alamat}
                      </span>
                    ...
                    <div className="text-gray-700 text-sm leading-snug">
                      {selectedAddressDetails.kelurahan},{" "}
                      {selectedAddressDetails.kecamatan},{" "}
                      {selectedAddressDetails.kabupaten},{" "}
                      {selectedAddressDetails.provinsi}
                    </div>
                  </div>
                  <button
                ...
                <div className="flex flex-col items-center justify-center py-8 border border-dashed border-gray-300 rounded-md">
                  <p className="text-gray-500 mb-4">
                    Kamu tidak mempunyai alamat pengiriman yang tersimpan
                  </p>
                  <button
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm sticky top-6">
                  ...
                  {checkoutItems.length > 0 ? (
                    checkoutItems.map((item, idx) => (
                      <div
                        ...
                        className="flex justify-between text-gray-700 mb-1 text-xs"
                      >
                        <span className="truncate max-w-[150px]">
                          {item.nama} {item.size ? `(${item.size})` : ""} ×
                          {item.quantity}
                        </span>
                        <span>
                          {formatPrice(parseFloat(item.harga) * item.quantity)}
                        </span>
                      </div>
                    ))
                  ) : (
              ...
              <div className="mt-4">
                <button
                  ...
                  disabled={
                    isProcessing || checkoutItems.length === 0 || !formValid
                  }
