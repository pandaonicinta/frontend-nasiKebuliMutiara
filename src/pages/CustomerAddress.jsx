...
const CustomerAddress = () => {
  ...
  const apiUrl = "http://eggsperts.my.id/api";
    ...
    try {
      ...
      if (err.response) {
        if (err.response.status === 401) {
          ...
          err.response.data?.message === "Kamu belum ada alamat utama"
        ) {
          setAddresses([]);
          setError(null);
        ...
        if (err.response.status === 401) {
          ...
          setError(
            `Cannot delete address: ${
              err.response.data?.message || "This address cannot be deleted"
            }`
          );
        } else {
          setError(
    ...
    try {
      ...
      if (currentAddress.id !== null && currentAddress.id !== undefined) {
        ...
        if (currentAddress.is_utama === 1) {
          await axios.get(`${apiUrl}/alamat/primary/${currentAddress.id}`, {
            headers,
          });
        }
      } else {
        const response = await axios.post(`${apiUrl}/alamat/add`, payload, {
          headers,
        });
  ...
  const handleBack = () => {
    if (
      isEditing &&
      (currentAddress.label_alamat || currentAddress.detail) &&
      !window.confirm(
                ...
                addresses.map((address) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-3 right-3 flex space-x-2">
                      ...
                      <button
                        onClick={() =>
                          handleDeleteAddress(address.id, address.is_utama)
                        }
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Phone number is required by the system
                  </p>
                </div>
                  ...
                  <div>
                    ...
                    {currentAddress.latitude && currentAddress.longitude && (
                      <p className="text-sm text-gray-700 mt-2">
                        Selected Location: {currentAddress.latitude},{" "}
