  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        setError(
          err.message || "Failed to load order history. Please try again later."
        );
        setOrders([]);
      } finally {
        setLoading(false);
            ...
            orders.map((order, index) => {
              ...
              const totalHarga = order.total_harga
                ...
                : order.keranjang?.reduce(
                    (sum, item) =>
                      sum +
                      (Number(item.harga) || 0) * (Number(item.quantity) || 1),
                    0
                  ) || 0;
              const displayItem =
                        ...
                        {displayItem ? (
                          <div className="w-full flex items-center mt-2">
                            ...
                            <div className="flex-1">
                              <h4 className="text-xs font-bold">
                                ...
                                <span className="text-gray-400 font-normal ml-1">
                                  x{displayItem.quantity || 1}
                                </span>
                              </h4>
                              <p className="text-xs text-gray-500">
                                Ukuran: {displayItem.ukuran}
                              </p>
                              {order.keranjang &&
                                order.keranjang.length > 1 && (
                                  <p className="text-xs text-gray-400">
                                    +{order.keranjang.length - 1} menu lain
                                    {order.keranjang.length > 2 ? "s" : ""}
                                  </p>
                                )}
                            </div>
                            <div className="text-center text-xs font-bold w-28">
                              Rp.{" "}
                              {(Number(displayItem.harga) || 0).toLocaleString(
                                "id-ID"
                              )}
                            </div>
                            <div className="w-28 text-right">
                              <button
                                onClick={() =>
                                  handleViewDetails(getOrderId(order))
                                }
